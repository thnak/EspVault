/**
 * @file vault_protocol.c
 * @brief Binary Protocol Implementation for EspVault Universal Node
 */

#include "vault_protocol.h"
#include <string.h>

/**
 * @brief CRC8 lookup table for fast calculation (polynomial 0x07)
 */
static const uint8_t crc8_table[256] = {
    
};

uint8_t vault_protocol_calc_crc8(const uint8_t *data, size_t len)
{
    uint8_t crc = 0x00;
    for (size_t i = 0; i < len; i++) {
        crc = crc8_table[crc ^ data[i]];
    }
    return crc;
}

void vault_protocol_init_packet(vault_packet_t *packet, uint8_t cmd, uint32_t seq)
{
    memset(packet, 0, sizeof(vault_packet_t));
    packet->head = VAULT_PROTO_HEAD;
    packet->cmd = cmd;
    packet->seq = seq;
}

void vault_protocol_finalize_packet(vault_packet_t *packet)
{
    // Calculate CRC over all bytes except the CRC field itself
    packet->crc = vault_protocol_calc_crc8((const uint8_t *)packet, 
                                           VAULT_PROTO_PACKET_SIZE - 1);
}

bool vault_protocol_validate_packet(const vault_packet_t *packet)
{
    // Check start of frame marker
    if (packet->head != VAULT_PROTO_HEAD) {
        return false;
    }
    
    // Verify CRC
    uint8_t calculated_crc = vault_protocol_calc_crc8((const uint8_t *)packet,
                                                       VAULT_PROTO_PACKET_SIZE - 1);
    return (calculated_crc == packet->crc);
}

bool vault_protocol_parse(const uint8_t *data, size_t len, vault_packet_t *packet)
{
    if (data == NULL || packet == NULL || len != VAULT_PROTO_PACKET_SIZE) {
        return false;
    }
    
    // Direct memory copy since structure is packed
    memcpy(packet, data, VAULT_PROTO_PACKET_SIZE);
    
    // Validate the parsed packet
    return vault_protocol_validate_packet(packet);
}

size_t vault_protocol_serialize(const vault_packet_t *packet, uint8_t *data)
{
    if (packet == NULL || data == NULL) {
        return 0;
    }
    
    // Direct memory copy since structure is packed
    memcpy(data, packet, VAULT_PROTO_PACKET_SIZE);
    
    return VAULT_PROTO_PACKET_SIZE;
}
